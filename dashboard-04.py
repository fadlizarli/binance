...
# ─── Cache Binance client (tidak reconnect setiap request) ───
_client = None
def get_client():
    global _client
    if _client is None:
...
def extract_state(logs):
    ...
    for line in logs:
        ...
        # Entry posisi — baca SL/TP dari log
    ...
    # Bot running — log terbaru dalam 3 menit?
    if logs:
...
# ─── Override dengan data real-time dari Binance ───
def enrich_with_binance(state):
    ...
    try:
        # Balance real (walletBalance bukan availableBalance)
...
<style>
  :root {
    --bg:#090c10; --card:#111820; --border:#1e2d3d;
    ...
    <table style="width:100%;border-collapse:collapse;font-family:'Space Mono',monospace;font-size:10px">
      <thead>
        <tr style="border-bottom:1px solid var(--border)">
          <th style="padding:10px 12px;text-align:left;color:var(--dim);letter-spacing:1px">TANGGAL</th>
...
async function refresh() {
  try {
    ...
    document.getElementById('wrsub_today').textContent = d.trades_today
      ? (d.wins_today||0)+'W '+(d.losses_today||0)+'L'
