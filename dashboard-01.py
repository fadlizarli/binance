...
import glob
LOG_FILE = sorted(glob.glob("logs/cryptobot_*.log"))[-1] if glob.glob("logs/cryptobot_*.log") else "logs/bot.log"
...
def extract_state_from_logs(logs):
    ...
    for line in reversed(logs):
        # Balance
        if "Balance:" in line and "$" in line and "balance" not in state.get("_found", ""):
            try:
                bal = line.split("$")[1].split()[0].replace(",", "")
                state["balance"] = float(bal)
                state.setdefault("_found", "")
                state["_found"] += "balance"
            except: pass
