...
def parse_trades():
    """Baca semua trade dari cryptobot_*.log"""
    import re as re2
    trades = []
    pattern = os.path.join(BASE_DIR, "logs", "cryptobot_*.log")
    files   = sorted(glob.glob(pattern))
                    ...
                    if i + 1 < len(lines):
                        ...
                        if entry_m and exit_m:
                            trades.append({
                                ...
                                "pnl":    round(
                                            float(pnl_m.group(1)) / 3
                                            if date < "2026-03-21 00:00"
                                            else float(pnl_m.group(1))
                                        , 2) if pnl_m else 0,
                                "reason": reason,
                            })
                i += 1
...
def extract_state(logs):
    ...
    for line in logs:
        m = re.search(r"Balance:\s*\$([\d,]+\.?\d*) USDT", line)
        ...
        if "Entry:" in line and "SL:" in line and "TP:" in line:
            ...
            if entry and last_side:
                state["position"] = {"side": last_side, "entry": entry, "sl": sl, "tp": tp, "symbol": state["symbol"]}
...
<div class="grid">
  ...
  <div class="card">
    ...
    <div class="sub">Dari $5,000 awal</div>
  </div>
  <div class="card">
    <div class="lbl">Unrealized PnL</div>
    <div class="val" id="upnl">+$0.00</div>
    <div class="sub" id="liq">Liq: —</div>
  </div>
  <div class="card y">
    <div class="lbl">Strategi</div>
    <div class="val y" id="strat" style="font-size:13px">—</div>
    <div class="sub" id="symsub">—</div>
  </div>
  <div class="card">
    <div class="lbl">Trade Hari Ini</div>
    <div class="val" id="trades_today">0</div>
    <div class="sub" id="wrsub_today">Belum ada trade</div>
  </div>
  <div class="card">
    <div class="lbl">Total Semua Trade</div>
    <div class="val" id="trades">0</div>
    <div class="sub" id="wrsub">Kumulatif</div>
  </div>
</div>
...
async function loadTrades() {
  try {
    ...
    tb.innerHTML = d.trades.slice(0,5).map(t => {
      ...
      const pnl = parseFloat(t.pnl)||0;
      const result = t.reason==='TAKE_PROFIT'?'✅ TP':t.reason==='STOP_LOSS'&&pnl>0?'🔄 TS':t.reason==='STOP_LOSS'?'❌ SL':t.reason;
      ...
      return '<tr style="border-bottom:1px solid var(--border)">' +
        ...
        '<td style="padding:8px;text-align:right;color:'+pnlColor+';font-weight:700">'+(pnl>=0?'+':'')+fmt(Math.abs(pnl))+'</td>'+
        '<td style="padding:8px;text-align:center;color:var(--dim)">'+result+'</td>'+
        '</tr>';
    }).join('');
  } catch(e) { console.error('Error load trades:', e); }
}
...
async function refresh() {
  try {
    ...
    document.getElementById('wrsub_today').textContent = d.trades_today
      ?(d.wins_today||0)+'W '+(d.losses_today||0)+'L':'Belum ada trade hari ini';
    document.getElementById('trades').textContent = d.total_trades||0;
    document.getElementById('wrsub').textContent = d.total_trades
      ?d.win_count+'W '+d.loss_count+'L · '+d.win_rate+'%':'Belum ada trade';
