...
class BinanceClient:
    ...
    # URL demo.binance.com (pengganti testnet lama)
    DEMO_BASE_URL    = "https://testnet.binancefuture.com"
    DEMO_FUTURES_URL = "https://dapi.binance.com"
    ...
    def _connect(self):
        ...
        try:
            if self.is_testnet:
                # demo.binance.com menggunakan endpoint yang sama dengan
                # testnet lama — cukup set testnet=True di python-binance
                self.client = Client(
                    self.api_key,
                    self.secret_key,
                    testnet=True,
                )
                # Override URL ke demo.binance.com jika testnet lama tidak jalan
                try:
                    self.client.futures_account_balance()
                    logger.info("✅ Terhubung ke Binance TESTNET/DEMO")
                except Exception:
                    # Coba endpoint demo langsung
                    self.client = Client(
                        self.api_key,
                        self.secret_key,
                    )
                    self.client.FUTURES_URL = "https://testnet.binancefuture.com/fapi"
                    logger.info("✅ Terhubung ke Binance DEMO (demo.binance.com)")
            else:
    ...
    def place_stop_loss_order(self, symbol: str, side: str,
        ...
        Demo Binance tidak support STOP_MARKET via endpoint biasa —
        gunakan closePosition=True sebagai fallback.
        """
        if not self.is_connected():
            return None
        # Coba STOP_MARKET dengan closePosition=True (kompatibel demo Binance)
        for params in [
            dict(type="STOP_MARKET", closePosition=True,  stopPrice=round(stop_price, 2)),
            dict(type="STOP_MARKET", closePosition=False, stopPrice=round(stop_price, 2),
                 quantity=round(quantity, 3), reduceOnly=True),
        ]:
            try:
                order = self.client.futures_create_order(
                    symbol=symbol, side=side, **params)
                logger.info(f"🛑 Stop Loss terpasang | {side} @ ${stop_price:,.2f} | ID:{order['orderId']}")
                return order
            except Exception as e:
                logger.warning(f"SL attempt gagal ({params['closePosition']}): {e}")
        logger.error("❌ Gagal pasang SL setelah 2 percobaan — SL dikelola secara software")
        return None
    ...
    def place_take_profit_order(self, symbol: str, side: str,
        ...
        Demo Binance tidak support TAKE_PROFIT_MARKET via endpoint biasa —
        gunakan closePosition=True sebagai fallback.
        """
        if not self.is_connected():
            return None
        for params in [
            dict(type="TAKE_PROFIT_MARKET", closePosition=True,  stopPrice=round(tp_price, 2)),
            dict(type="TAKE_PROFIT_MARKET", closePosition=False, stopPrice=round(tp_price, 2),
                 quantity=round(quantity, 3), reduceOnly=True),
        ]:
            try:
                order = self.client.futures_create_order(
                    symbol=symbol, side=side, **params)
                logger.info(f"🎯 Take Profit terpasang | {side} @ ${tp_price:,.2f} | ID:{order['orderId']}")
                return order
            except Exception as e:
                logger.warning(f"TP attempt gagal ({params['closePosition']}): {e}")
        logger.error("❌ Gagal pasang TP setelah 2 percobaan — TP dikelola secara software")
        return None
